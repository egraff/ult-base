...
def _convert_pdf_page_to_png_async(pdf_path: str, page_num: int, output_png_path: str) -> Awaitable[Tuple[int, List[str], List[str]]]:
  gs_cmd = [
    GS,
    '-q',
    '-dQUIET',
...
async def compare_pngs_async(png_path_first: str, png_path_second: str, output_diff_path: str) -> Awaitable[int]:
  cmp_cmd = [CMP, '-metric', 'ae', png_path_first, png_path_second, 'PNG24:%s' % output_diff_path]
...
class PdfFile(object):
  ...
  async def _determine_num_pages_in_pdf_async(path: str) -> Awaitable[int]:
    ...
    returncode, stdout, _stderr = await asynclib.popen_async([PDFINFO, path], timeout=3)
  ...
  async def get_png_for_page_async(self, page_num: int, output_png_path: str) -> Awaitable[Tuple[int, List[str], List[str]]]:
    assert page_num >= 1
    assert page_num <= self._num_pages

    return await _convert_pdf_page_to_png_async(self.path, page_num, output_png_path)
