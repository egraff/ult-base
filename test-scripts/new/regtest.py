...
async def test_pdf_page_pair_async(
  ...
  async with config.process_pool_semaphore:
    test_pdf_task = asyncio.ensure_future(test_pdf_obj.get_png_for_page_async(page_num, test_png_page_path))
    proto_pdf_task = asyncio.ensure_future(proto_pdf_obj.get_png_for_page_async(page_num, proto_png_page_path))
    ...
    assert gen_test_png_returncode == 0, "Failed to generate PNG %s" % (test_png_page_path,)
    assert gen_proto_png_returncode == 0, "Failed to generate PNG %s" % (proto_png_page_path,)
    ...
    ae_diff = await compare_pngs_async(test_png_page_path, proto_png_page_path, diff_path)
...
async def test_pdf_pair_async(config: TestConfig, test_name: str) -> Awaitable[Tuple[str, List[int]]]:
  ...
  for page_num in page_list:
    ...
    task = asyncio.ensure_future(test_pdf_page_pair_async(config, test_pdf_obj, proto_pdf_obj, page_num, test_name))
...
async def make_test_tex_file_async(config: TestConfig, test_name: str) -> Awaitable[Tuple[int, List[str], List[str]]]:
  cmd = ['make', '-C', config.TESTDIR, '--no-print-directory', '_file', 'RETAINBUILDFLD=y', 'FILE=%s.tex' % (test_name,)]
...
async def run_test_async(config: TestConfig, test_name: str) -> Awaitable[TestResult]:
  async with config.make_task_semaphore:
    try:
      returncode, stdout, stderr = await make_test_tex_file_async(config, test_name)
...
class TestRunner:
  ...
  def echo(self, *string):
    ...
    with self.config.echo_lock:
      subprocess.Popen([
        'sh',
        '-c',
        'printf "{}"; printf "{}"; printf "{}"'.format(color, echo_str.encode('unicode_escape').decode("utf-8").replace('"', r'\"'), '\\033[0m')
      ]).wait()

  async def _run_test(self, test_name):
    test_result = await run_test_async(self.config, test_name)
    test_passed = test_result.build_succeeded and (test_result.exc_info is None) and (len(test_result.failed_pages) == 0)
  ...
  async def run(self, test_names):
    ...
    async with self.test_result_lock:
      with open(os.path.join(self.config.TESTDIR, "test_result.json").replace("\\", "/"), "w", encoding="utf8") as fp:
        ...
        for test_result in self.failed_tests:
          ...
          failed_test_map['exception'] = False if test_result.exc_info is None else True
          ...
          if test_result.build_timed_out:
            ...
            latex_log_file = ".build/%s/output.log" % (test_result.test_name,)
            ...
            failed_test_map['exc_info']['type'] = str(test_result.exc_info[0])
            failed_test_map['exc_info']['value'] = str(test_result.exc_info[1])
            ...
            failed_pages_string = ", ".join(str(x) for x in test_result.failed_pages)
...
def test_generator(tex_tests_root_dir: str, test_file_prefix='test'):
  for dir_path, dir_names, file_names in os.walk(tex_tests_root_dir):
    for file_name in file_names:
      ...
      test_name = os.path.relpath(os.path.join(dir_path, filebasename), tex_tests_root_dir).replace("\\", "/")
