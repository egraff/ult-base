...
class TestConfig():
  def __init__(self, testDir, numDotsPerLine=80, testFilePrefix="test", protoFilePrefix="proto", debugLevel=debug.INFO):
    testDir = os.path.relpath(os.path.realpath(testDir))
    assert os.path.isdir(testDir)

    self.TESTDIR     = testDir
    self.TEXTESTDIR  = os.path.join(testDir, "tests").replace("\\", "/")
    ...
    self.PROTOFILEPREFIX = protoFilePrefix
...
if __name__ == '__main__':
  ...
  runner.run(testGenerator(config))
