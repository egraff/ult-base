...
RUN \
  ...
  apt-get install --no-install-recommends -qq -y libfile-fcntllock-perl gcc equivs libwww-perl fontconfig unzip
...
RUN \
  export INSTALL_TL_REPO=https://ftp.math.utah.edu/pub/tex/historic/systems/texlive/2022/tlnet-final && \
...
RUN \
  export TLNET_REPO=http://ftp.math.utah.edu/pub/tex/historic/systems/texlive/2022/tlnet-final ; \
  ...
  tlmgr install \
    ...
    collection-pstricks \
  ; while [ $? -ne 0 ]; do !!; done ; \
  export TLNET_REPO=https://ftp.math.utah.edu/pub/tex/historic/systems/texlive/2022/tlnet-final ; \
  tlmgr repository set ${TLNET_REPO} ; \
