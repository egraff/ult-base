...
$httpsRepo = 'https://ftp.math.utah.edu/pub/tex/historic/systems/texlive/2024/tlnet-final'
$httpRepo = 'http://ftp.math.utah.edu/pub/tex/historic/systems/texlive/2024/tlnet-final'

$scriptRoot = (Resolve-Path $(If ($PSScriptRoot) { $PSScriptRoot } Else { "." })).Path
. "${scriptRoot}\install_texlive_windows.ps1" `
    -Repository $httpsRepo `
    -ProfilePath "${scriptRoot}\texlive2024-win.profile"
